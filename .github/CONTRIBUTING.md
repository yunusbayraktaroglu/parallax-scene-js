# Contributing to `parallax-scene-js`

Thanks for wanting to contribute! It's more than welcome.

As the creators and maintainers of this project, we want to ensure that `parallax-scene-js` lives and continues to grow and evolve. We would like to encourage everyone to help and support this library by contributing

## Project philosophy

Design and render raw WebGL parallax scenes using a single draw call.
The goal is to achieve smooth, visually rich parallax effects that perform efficiently even on low-end mobile devices.
...
5. **Contribute the to code.**
   ...
   Code-level contributions to `parallax-scene-js` like creating or fixing a hook generally come in the form of pull requests. These are done by forking the repo and making changes locally explained below.
...
2. Fork and clone the repo to your local machine:
   ...
   ```shellscript
   git clone https://github.com/yunusbayraktaroglu/parallax-scene-js
   ```
...
9. Submit a pull request to the upstream `parallax-scene-js` repository.

10. Choose a descriptive title and describe your changes briefly.

## Coding style

Please follow the coding style of the project. `parallax-scene-js` uses eslint and prettier. If possible, enable their respective plugins in your editor to get real-time feedback. The linting can be run manually with the following command: `npm lint` and `npm prettier`.