...
export type ImageMergeResult = MergeResult & {
	/**
	 * Deterministic hash generated by using array of image paths
	 */
	hash: string;
};
...
export class ResourceController
{
	...
	createTexture( hash: string, imageBitmap: ImageBitmap, options?: TextureOptions )
	{
		...
		if ( texture ){
			console.warn( `ResourceController: '${ hash }' texture has was created previously` );
			return texture;
		}
	...
	add( key: string, imageBitmap: ImageBitmap )
	{
		if ( this._images.has( key ) ){
			console.warn( `ResourceController: '${ key }' already in the image bitmaps resources.` );
			return;
		}
	 ...
	 */
	async merge( imageBitmaps: ImageDownloadResult[], canvasSettings: CanvasRenderingContext2DSettings = { alpha: true } ): Promise<ImageMergeResult>
	{
		...
		try {
			...
			// DEBUG: Appends texture in document.body to debug
	...
	{
		this._texturePacker._displayImageBitmapOnScreen( image, { alpha: true } );
	}
